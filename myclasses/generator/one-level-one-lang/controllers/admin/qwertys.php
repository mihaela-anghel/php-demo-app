		...
		*/		
		...
		{					
			...
			if(isset($search["qwerty_id"]) && !empty($search["qwerty_id"])) 
				$where .= " AND qwerty_id = '".$search["qwerty_id"]."' ";
			...
			{
				$where .= " AND LOWER(name) LIKE '%".strtolower(mysql_real_escape_string($search["name"]))."%' ";										
			}									
		}																							
		...
		//pagination
		//======================================================						
		$rows 					= $this->qwertys_model->get_qwertys($where,false,false,false,"count(qwerty_id) as nr");		
		...
		foreach($qwertys as $key => $qwerty)
		{
			...
			$images 						= $this->qwertys_model->get_images($where,$orderby,1,0,"filename");
			if($images)
				$qwertys[$key]["image"]		= $images[0]["filename"];
			*/			
