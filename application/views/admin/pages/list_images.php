...
$(document).ready(function() {
	$('#file').uploadifive({
		'uploadScript'    	: '<?php echo admin_url()?>pages/images_upload/<?php echo $page['page_id']?>',				
		'auto'      		: true,		
		'multi'     		: true,
...
<table class="list_table">
    ...
    foreach($images as $image)
    {
        ...
        <tr>
            <td>
            	<a href="<?php echo file_url()?>uploads/pages/images/<?php echo $image['filename']?>?<?php echo uniqid('')?>" class="fancybox_image">
                	<img src="<?php echo file_url()?>uploads/pages/images/<?php echo $image['filename']?>?<?php echo uniqid('')?>" width="100"/>
                </a>
			</td>
            <td>
                <input type="text" value="<?php echo $image["order"]?>" onblur="window.location='<?php echo admin_url()?>pages/images_change/<?php echo $image["image_id"] ?>/order/'+this.value" style="width:30px;"/>
            </td>            
            <td>
				<?php
                //active				
                $aux = array(	"field" 	=> "active",
                                ...
                                "url" 		=> admin_url()."pages/images_change/".$image["image_id"]	
                                );
            ...
            <td>	
                ...
                {
                    ?>
                    <a href="javascript:void(0)" onclick="if(confirm('<?php echo $this->lang->line("confirm_delete_file")?>')) window.location='<?php echo admin_url()?>pages/images_delete/<?php echo $image["image_id"] ?>'" class="delete">
