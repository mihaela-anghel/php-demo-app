...
$(document).ready(function() {
	$('#file').uploadifive({
		'uploadScript'    	: '<?php echo admin_url()?>pages/files_upload/<?php echo $page['page_id']?>',				
		'auto'      		: true,		
		'multi'     		: true,
...
<table class="list_table">
    ...
    foreach($files as $file)
    {
        ...
        <tr>
            <td>
            	<a href="<?php echo file_url()?>uploads/pages/files/<?php echo $file['filename']?>" target="_blank" class="fancybox_file">
                	<?php echo $file['filename']?>
                </a>
            </td>
            <td>   
                <span><?php echo $this->lang->line('page_file_name')?></span>             
                
                <input type="text" name="name[<?php echo $file['file_id']?>]" value="<?php echo $file["name"]?>">
                <input type="submit" name="SaveButton[<?php echo $file['file_id']?>]" value="<?php echo $this->lang->line('save')?>">

                <?php echo form_error("name[".$file['file_id']."]")?>
            </td>
            <td>
                <input type="text" value="<?php echo $file["order"]?>" onblur="window.location='<?php echo admin_url()?>pages/files_change/<?php echo $file["file_id"] ?>/order/'+this.value" style="width:30px;"/>
            </td>            
            <td>
				<?php
                //active				
                $aux = array(	"field" 	=> "active",
                                ...
                                "url" 		=> admin_url()."pages/files_change/".$file["file_id"]	
                                );
            ...
            <td>	
                ...
                {
                    ?>
                    <a href="javascript:void(0)" onclick="if(confirm('<?php echo $this->lang->line("confirm_delete_file")?>')) window.location='<?php echo admin_url()?>pages/files_delete/<?php echo $file["file_id"] ?>'" class="delete">
