    ...
    <div id="array_div_<?php echo $key?>" style="clear:left; display:<?php if($key == 0) echo "block"; else echo "none";?>" >	
		...
		<table class="form_table tab" width="<?php echo $table_width?>">
			<tr>
				...
				<td>
                	<input type="text" name="right_name[<?php echo $language["lang_id"]; ?>]" id="right_name_<?php echo $language["lang_id"]; ?>" value="<?php echo set_value("right_name[".$language["lang_id"]."]",(isset($right_details["admin_right_name"][$language["lang_id"]])?$right_details["admin_right_name"][$language["lang_id"]]:"")); ?>"/>*
                </td>
			</tr> 						
		</table>
		</div> 
...
<table class="form_table" width="<?php echo $table_width?>">  
	<tr>
		...
		<td>
            ...
            foreach($class_methods as $key=>$value)
            {
                ...
                {
                    if($value == $right["admin_right_url"]) $selected = true; else $selected = false;
	...
	<tr>
		...
		<td>
        	<input type="text" name="order" id="order" value="<?php echo set_value("order",$right["order"]); ?>" style="width:30px;"/>
		</td>
	</tr>
	<tr>
		<th>
		...
		<td>
			...
			{
				if($right["active"] == $value) $selected = true; else $selected = false;
