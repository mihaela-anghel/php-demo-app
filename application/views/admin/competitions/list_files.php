...
$(document).ready(function() {
	$('#file').uploadifive({
		'uploadScript'    	: '<?php echo admin_url()?>competitions/files_upload/<?php echo $competition['competition_id']?>',				
		'auto'      		: true,		
		'multi'     		: true,
...
<table class="list_table">
    ...
    foreach($files as $file)
    {
        ...
        <tr>
            <td>
            	<a href="<?php echo file_url()?>uploads/competitions/files/<?php echo $file['filename']?>" target="_blank" class="fancybox_file">
                	<?php echo $file['filename']?>
                </a>
			</td>
            <td>
                <input type="text" value="<?php echo $file["order"]?>" onblur="window.location='<?php echo admin_url()?>competitions/files_change/<?php echo $file["file_id"] ?>/order/'+this.value" style="width:30px;"/>
            </td>            
            <td>
				<?php
                //active				
                $aux = array(	"field" 	=> "active",
                                ...
                                "url" 		=> admin_url()."competitions/files_change/".$file["file_id"]	
                                );
            ...
            <td>	
                ...
                {
                    ?>
                    <a href="javascript:void(0)" onclick="if(confirm('<?php echo $this->lang->line("confirm_delete_file")?>')) window.location='<?php echo admin_url()?>competitions/files_delete/<?php echo $file["file_id"] ?>'" class="delete">
