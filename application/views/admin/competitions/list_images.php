...
$(document).ready(function() {
	$('#file').uploadifive({
		'uploadScript'    	: '<?php echo admin_url()?>competitions/images_upload/<?php echo $competition['competition_id']?>',				
		'auto'      		: true,		
		'multi'     		: true,
...
<table>
    <tr>
		...
		<td>
            <div class="small">JPG, GIF, PNG, max 3 Mb, 850x430 pixeli</div>
        </td>
    </tr>
</table>
...
<table class="list_table">
    ...
    foreach($images as $image)
    {
        ...
        <tr>
            <td>
            	<a href="<?php echo file_url()?>uploads/competitions/images/<?php echo $image['filename']?>?<?php echo uniqid('')?>" class="fancybox_image">
                	<img src="<?php echo file_url()?>uploads/competitions/images/<?php echo $image['filename']?>?<?php echo uniqid('')?>" width="100"/>
                </a>
			</td>
            <td>
                <input type="text" value="<?php echo $image["order"]?>" onblur="window.location='<?php echo admin_url()?>competitions/images_change/<?php echo $image["image_id"] ?>/order/'+this.value" style="width:30px;"/>
            </td>            
            <td>
				<?php
                //active				
                $aux = array(	"field" 	=> "active",
                                ...
                                "url" 		=> admin_url()."competitions/images_change/".$image["image_id"]	
                                );
            ...
            <td>	
                ...
                {
                    ?>
                    <a href="javascript:void(0)" onclick="if(confirm('<?php echo $this->lang->line("confirm_delete_file")?>')) window.location='<?php echo admin_url()?>competitions/images_delete/<?php echo $image["image_id"] ?>'" class="delete">
