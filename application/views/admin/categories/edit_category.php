...
<table class="form_table" width="<?php echo $table_width?>">  
	<tr style="display:none">
        ...
        <td>
            ...
            foreach($categories as $value)
            {
                ...
                if($category['parent_id'] == $value['category_id']) $default = TRUE; else $default = FALSE;
                ?><option value="<?php echo $value['category_id']?>" <?php echo set_select('parent_id',$value['category_id'],$default); ?>><?php for($i = 0; $i < ($value['level']*1) ; $i++) echo "---";  echo $value['category_name']?></option><?php
            }
            ?>
            </select>
    ...
    <tr>
		...
		<td>
			...
			{
				if($category["active"] == $value) $selected = true; else $selected = false;
		...
		<div style="clear:left;">
			<table class="form_table tab" width="<?php echo $table_width?>">
                <tr>
                    ...
                    <td>
                    	<input type="text" name="name[<?php echo $language['lang_id']; ?>]" id="name_<?php echo $language['lang_id']; ?>" value="<?php echo set_value('name['.$language['lang_id'].']',@$category_details['category_name'][$language['lang_id']]); ?>" style="width:600px;"/>
                    </td>
                </tr>   
                <tr style="display:none">
                    <th><?php echo $this->lang->line('category_description').$label_language?></th>
                    <td>			
                    	<textarea name="description[<?php echo $language['lang_id']; ?>]" id="description_<?php echo $language['lang_id']; ?>" rows="4" class="html_textarea" style="width:600px; height:300px"><?php echo set_value('description['.$language['lang_id'].']',@$category_details['category_description'][$language['lang_id']]); ?></textarea>
                    </td>
                </tr>  
                <tr style="display:none">
                    <th><?php echo $this->lang->line('meta_description').$label_language?></th>
                    <td>			
                    	<textarea name="meta_description[<?php echo $language['lang_id']; ?>]" id="meta_description_<?php echo $language['lang_id']; ?>" style="width:600px;" rows="4"><?php echo set_value('meta_description['.$language['lang_id'].']',@$category_details['meta_description'][$language['lang_id']]); ?></textarea>
                    </td>
                </tr>  
                <tr style="display:none">
                    <th><?php echo $this->lang->line('meta_keywords').$label_language?></th>
                    <td>			
                    	<textarea name="meta_keywords[<?php echo $language['lang_id']; ?>]" id="meta_keywords_<?php echo $language['lang_id']; ?>" style="width:600px;" rows="4"><?php echo set_value('meta_keywords['.$language['lang_id'].']',@$category_details['meta_keywords'][$language['lang_id']]); ?></textarea>
                    </td>
                </tr>  
			</table>
		</div> 
