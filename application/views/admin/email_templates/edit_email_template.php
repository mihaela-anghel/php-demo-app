...
<table class="form_table" width="<?php echo $table_width?>" <?php if($_SESSION["admin_auth"]["admin_role"] != "webmaster") { ?>style="display:none"<?php }?>>  		
	<tr>
        <th width="<?php echo $th_width?>">
            <?php echo $this->lang->line("email_template_identifier"); ?>
        </th>
        <td>
            <?php echo form_error('identifier'); ?>
            <input type="text" name="identifier" id="identifier" value="<?php echo set_value("identifier",$email_template["identifier"]); ?>"/>
        </td>
    </tr>    	
</table>
    ...
    <div id="array_div_<?php echo $key?>" style="clear:left; display:<?php if($key == 0) echo "block"; else echo "none";?>" >	
		...
		<div style="clear:left;">
            <table class="form_table tab" width="<?php echo $table_width?>">    
                <tr>
                    <th width="<?php echo $th_width?>">
                        <?php echo $this->lang->line("email_template_name").$label_language?>
                    </th>
                    <td>
                        <input type="text" name="name[<?php echo $language["lang_id"]; ?>]" id="name_<?php echo $language["lang_id"]; ?>" value="<?php echo set_value("name[".$language["lang_id"]."]",(isset($email_template_details["name"][$language["lang_id"]])?$email_template_details["name"][$language["lang_id"]]:"")); ?>" style="width:600px;"/>*
                    </td>
                </tr>             
                <tr>
                    <th width="<?php echo $th_width?>">
                        <?php echo $this->lang->line("email_template_description").$label_language?>
                    </th>
                    <td>
                    	<textarea name="description[<?php echo $language["lang_id"]; ?>]" id="description_<?php echo $language["lang_id"]; ?>"  class="html_textarea" rows="4" class="" style="width:600px; height:150px"><?php echo set_value("description[".$language["lang_id"]."]",(isset($email_template_details["description"][$language["lang_id"]])?$email_template_details["description"][$language["lang_id"]]:"")); ?></textarea>                        						
                    </td>
                </tr>                					
            </table>
		</div> 
