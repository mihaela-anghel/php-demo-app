...
<table class="form_table" width="<?php echo $table_width?>">  
	<tr style="display:none">
        <th width="<?php echo $th_width?>"><?php echo $this->lang->line('galleries_category')?></th>
        <td>
            <select name="parent_id" class="select">
            <option value="0"><?php echo $this->lang->line('select')?></option>
            <?php
            foreach($galleries_categories as $value)
            {
                for($i = 0; $i < ($value['level']*6) ; $i++)	$level .= "&nbsp;";	
                if($galleries_category['parent_id'] == $value['galleries_category_id']) $default = TRUE; else $default = FALSE;
                ?><option value="<?php echo $value['galleries_category_id']?>" <?php echo set_select('parent_id',$value['galleries_category_id'],$default); ?>><?php for($i = 0; $i < ($value['level']*1) ; $i++) echo "---";  echo $value['galleries_category_name']?></option><?php
            }
            ?>
            </select>
    ...
    <tr>
		...
		<td>
			...
			{
				if($galleries_category["active"] == $value) $selected = true; else $selected = false;
	...
	?><div id="array_div_<?php echo $key?>" style="clear:left; display:<?php if($key == 0) echo 'block'; else echo 'none';?>" ><?php
		...
		<div style="clear:left;">
			<table class="form_table tab" width="<?php echo $table_width?>">
                <tr>
                    <th width="<?php echo $th_width?>"><?php echo $this->lang->line('galleries_category_name').$label_language?>*</th>
                    <td>
                    	<input type="text" name="name[<?php echo $language['lang_id']; ?>]" id="name_<?php echo $language['lang_id']; ?>" value="<?php echo set_value('name['.$language['lang_id'].']',@$galleries_category_details['galleries_category_name'][$language['lang_id']]); ?>" style="width:600px;"/>
                    </td>
                </tr>   
                <tr style="display:none">
                    <th><?php echo $this->lang->line('galleries_category_description').$label_language?></th>
                    <td>			
                    	<textarea name="description[<?php echo $language['lang_id']; ?>]" id="description_<?php echo $language['lang_id']; ?>" rows="4" class="html_textarea" style="width:600px; height:300px"><?php echo set_value('description['.$language['lang_id'].']',@$galleries_category_details['galleries_category_description'][$language['lang_id']]); ?></textarea>
                    </td>
                </tr>  
                <tr style="display:none">
                    <th><?php echo $this->lang->line('meta_description').$label_language?></th>
                    <td>			
                    	<textarea name="meta_description[<?php echo $language['lang_id']; ?>]" id="meta_description_<?php echo $language['lang_id']; ?>" style="width:600px;" rows="4"><?php echo set_value('meta_description['.$language['lang_id'].']',@$galleries_category_details['meta_description'][$language['lang_id']]); ?></textarea>
                    </td>
                </tr>  
                <tr style="display:none">
                    <th><?php echo $this->lang->line('meta_keywords').$label_language?></th>
                    <td>			
                    	<textarea name="meta_keywords[<?php echo $language['lang_id']; ?>]" id="meta_keywords_<?php echo $language['lang_id']; ?>" style="width:600px;" rows="4"><?php echo set_value('meta_keywords['.$language['lang_id'].']',@$galleries_category_details['meta_keywords'][$language['lang_id']]); ?></textarea>
                    </td>
                </tr>  
			</table>
		</div> 
